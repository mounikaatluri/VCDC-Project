...
if ($result->num_rows > 0) {
    ...
    echo "<center><h2>Current Users</h2><div class=\"CSSTableGenerator\"><table cellspacing='20'><tr><td>FIRST NAME</td><td>LAST NAME</td><td>USER NAME</td><td>PASSWORD</td><td>EMAIL</td><td>PRIMARY ROLE</td><td>SECONDARY ROLE</td><td>LEAST IMPORTANT ROLE</td><td>DELETE</td></tr>";
    while($row = $result->fetch_assoc()) {
        echo "<tr><center><td>" . ucfirst($row["firstname"]). "</td><td>" . ucfirst($row["lastname"]). "</td><td>" . $row["username"]. "</td><td>" . $row["password"]."</td><td><a href=\"mailto:".$row["email"]."\">".$row["email"]."</a></td><td>" . $row["role1"]."</td><td>" . $row["role2"]."</td><td>" . $row["role3"]."<br></td><td>
        <form name=\"quries\" action=\"users.php\" method=\"post\"><input type=\"hidden\" name=\"id\" value=\"".$row["username"]."\" /><input type=\"submit\" name=\"delete\" class=\"sub\" value=\"delete\"></form></td></tr>";
